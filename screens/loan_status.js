...
const Loan_s = ({route, navigation}) => {
    const {selectedValue, selectedValueLoan, selectedValueDay, repayment, interest, date} = route.params;
    const [isExpanded, setIsExpanded] = useState(false);
    const [headerText1, setHeaderText1] = useState("Loan Amount");
    const [headerText2, setHeaderText2] = useState("Ksh 2000");
    const [headerText3, setHeaderText3] = useState("status");
    const [headerText4, setHeaderText4] = useState("Active");
    ...
    const handleHeaderChange = () => {
      if (isExpanded) {
        setHeaderText1("Loan Amount");
        setHeaderText2("Ksh " + selectedValueLoan);
    ...
    if (!selectedValue || !selectedValueLoan || !selectedValueDay || !repayment || !interest || !date){
        return(
            <View style={{backgroundColor: 'white', flex: 1, alignItems: 'center', justifyContent: 'center'}}>
                <Text style={{color: 'black'}}>You Have No Loan Records Yet</Text>
            </View>
        )
    }
        ...
        <View style={[styles.container, isExpanded && styles.expandedContainer]}>
          <View style={styles.header}>
            ...
            <View style={[styles.headerContent1, isExpanded ? styles.expandedHeaderContent1 : styles.headerContent1]}>
              ...
              <Text style={styles.headerText2('#14FA47')}>{" " + headerText2}</Text>
            </View>
            <View style={styles.headerContent2}>
              <Text style={styles.headerText3}>{headerText3}</Text>
              <Text style={styles.headerText2('#88D060')}>{headerText4}</Text>
            </View>
          </View>
        {isExpanded && (
          <View>
